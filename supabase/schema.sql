...
INSERT INTO time_dimension (date, year, month, week, day_of_week, quarter, month_name, is_weekend)
SELECT 
  d::date,
...
CREATE UNIQUE INDEX idx_uk_production_date_product ON production_summary (date, (COALESCE(product_code, '__ALL__')));
...
WITH defect_totals AS (
  SELECT 
    ...
    dm.category,
    dm.severity,
    SUM(doc.quantity) AS total_quantity,
    COUNT(DISTINCT doc.date) AS days_occurred
  FROM defect_occurrence doc
  JOIN defect_master dm ON doc.defect_id = dm.id
  WHERE doc.date >= CURRENT_DATE - INTERVAL '30 days'
  GROUP BY doc.defect_id, dm.code, dm.display_name, dm.category, dm.severity
),
grand_total AS (
